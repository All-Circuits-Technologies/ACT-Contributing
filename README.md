<!--
SPDX-FileCopyrightText: 2024 - 2026 Benoit Rolandeau <benoit.rolandeau@allcircuits.com>

SPDX-License-Identifier: LicenseRef-ALLCircuits-ACT-1.1
-->

# ACT Contributing

The canonical contributing rules and coding standards for ACT projects. Every ACT software
repository is expected to follow them.

## Contents

- **[CONTRIBUTING.md](CONTRIBUTING.md)** - how to contribute: licensing (REUSE), pull and merge
  requests. Read this before opening a request.
- **[CODE_OF_CONDUCT.md](CODE_OF_CONDUCT.md)** - the code of conduct all contributors follow.
- **[software/coding-standards.md](software/coding-standards.md)** - the coding standards to respect
  in our software repositories, with per-language rules (global, C, C++, Qt, Flutter).
